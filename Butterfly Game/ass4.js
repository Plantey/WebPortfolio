...
//PROJECT BREAKDOWN
    ...
    //NOTES: bars spawnrate increases as player score goes up, circles change colour upon torioding, makin sure that all colours are available to the player, mostly randomly
...
var state = {
    space: false, //is spacebar pressed
    spawnRects: false, //is spawnRects on (gamestart)
    game: true, //checking for gameover, true is alive!
} 
...
function spawnRects(){
    ...
    if(score == 3){
        gameSpeed = 9000;
        console.log("speed increase 1:", gameSpeed/1000, "second intervals")
    }else if(score == 6){
        gameSpeed = 7000;
        console.log("speed increase 2:", gameSpeed/1000, "second intervals")
    }else if(score == 9){
        gameSpeed = 6000;
        console.log("speed increase 3:", gameSpeed/1000, "second intervals")
    }else if(score == 12){
        gameSpeed = 5000;
        console.log("speed increase 4:", gameSpeed/1000, "second intervals")
    }
    else if(score == 15){
        gameSpeed = 4000;
        console.log("speed increase 5:", gameSpeed/1000, "second intervals")
    }
    else if(score == 18){
        gameSpeed = 3500;
        console.log("speed increase 6:", gameSpeed/1000, "second intervals")
    }else if(score == 40){
        gameSpeed = 3000;
        console.log("speed increase 7:", gameSpeed/1000, "second intervals")
    }else if(score == 70){
        gameSpeed = 2700;
        console.log("speed increase max:", gameSpeed/1000, "second intervals")
    }
...
//stops all change x and y values for all array items for my two array parameters
function collision_stopAll(o, array1, array2){
    for(var i=0; i<array1.length;i++){
        stop(o, array1[i], array1);
    }
    for(var i=0; i<array2.length;i++){
        stop(o, array2[i], array2);
    }
}
...
function drawScore(){ //citation 5 here, used for these lines of code
    //create text gradient
    const gradient = ctx.createLinearGradient(10, 45, 100, 100);
    gradient.addColorStop(0, "purple");
    gradient.addColorStop(0.5, "green");
    gradient.addColorStop(1.0, "orange");
    //create fontstyle, fillstyle(set to gradient)
...
function gameOver(){
    ...
    //gameover, plus r to restart
    ctx.font = "48px Georgia";
    //fill and stroke text, game over, your score: gameSpeed, refresh to try again
    ctx.fillStyle = "darkgreen";
    ctx.fillText("GAME OVER", canvw/2-135, canvh/2-60);
    ctx.strokeText("GAME OVER", canvw/2-135, canvh/2-60);
    ctx.font = "36px Georgia";
    ctx.fillText("Your Score: " + score, canvw/2-100, canvh/2-15);
    ctx.strokeText("Your Score: " + score, canvw/2-100, canvh/2-15);
    ctx.fillText("refresh ([ctrl] + [r]) to try again!", canvw/2-250, canvh/2+80);
    ctx.strokeText("refresh ([ctrl] + [r]) to try again!", canvw/2-250, canvh/2+80);

    //stopping array objects from moving, ensure score cannot increase or console log anything else after a game over
    collision_stopAll(butt, allCircles, allRects);
