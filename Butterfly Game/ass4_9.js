...
//checking colour picker function for butterfly
//console.log(getColour(allColours));
//console.log("butterfly starting colour -", butt.c)
...
//create rectangles, wait five seconds, then start spawning rectangles every x seconds
setTimeout(function(){
    state.spawnRects = true;  //starts rect spawning after giving the player two seconds of "tutorial time"
    console.log("Rects start");
}, 5000);
...
function animationLoop(){
    ...
    for (var i=0; i<allRects.length;i++){
        ...
        if(allRects[i].y+allRects[i].h/2 < -10){ //if bottom of rect (+half of height) rect goes above canvas
            allRects.splice(allRects[i], 1);
            console.log("rectangle removed");//logs square removal to canvas, use this for score??
            score++;
...
function collision_rects(o1, o2){
    if( //collision is detected and colours are the same
        o1.y+o1.h/2 > o2.y-o2.h/2 && //o1 bottom greater than o2 top
        o1.y-o1.h/2 < o2.y+o2.h/2 && //o1 top less than o2 bottom
        o1.c == o2.c 
    ){
        //lalala 
    }else if ( //collision is detected and colours are NOT the same 
        o1.y+o1.h/2 > o2.y-o2.h/2 &&
        o1.c != o2.c
    ){
        //stop(o1,o2); //execute stop function
        o1.y = o2.y - 10;
        //o1.y -= 5;
    }
}
